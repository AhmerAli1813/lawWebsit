...
$q = $db_conn->query("SELECT app_id ,C_id , client_name, case_type,date_booked, appointment_date, lawyer_status FROM `case_appointment` WHERE  `Admin_status` = 'approved' AND `lawyer_status` = 'accepted' ");
$output="";
...
if(mysqli_num_rows($q) >0){
        while($row = mysqli_fetch_assoc($q)){
                ...
                <td class='d-flex'><a href='action/action.php?del_id={$row['app_id']}' class='btn btn-danger' '><i class='fas fa-drum'></i></a><a href='lawyers_acc.php?edit={$row['app_id']}' id='edit_btn'   class='ml-1 btn btn-info'><i class='fas fa-book-open'></i></a></td>
                </tr>                
                ";
        }
}else{
